...
def initAllPhy(begin,nRobots):
	...
	for i in range(nRobots):
	  allPhy.append(Tree(stream=StringIO(str(begin)+'_' + str(i)+';'), schema="newick",rooted=True))
	  allPhy[i].reroot_at(allPhy[i].nodes()[0])
	  for node in allPhy[i].preorder_node_iter():
			    node.genome = ""
			    node.label = str(begin)+" " + str(i)
...
#Given a node, return the generation of this node
