...
echo "Iteration,alive,both,r0,r1,Sim,Sparsity,rep" > $destdir/$pref'_actives.csv'
...
#		 ' >> $destdir/$pref"_genometracking.csv"
	cat $i | grep active | awk  -v start=$start -v e=$end -v s=$sim  -v sp=$gSpars -v rep=$gRep '
	{
		if($1 < e && $1 > start )
		{
			print $1","$4","$5","$6","$7","s","sp","rep
		}
