...
plotReward<-function(n,b,resourceValue,dens=1,...){
g_skill=seq(-1,1,.01)
plot(g_skill,reward(g_skill,1,n,b,resourceValue,dens,...),type="l",col=myColors["1"],xlim=c(-1.1,1.1),ann=F,...)
points(g_skill,reward(g_skill,0,n,b,resourceValue,dens,...),type="l",col=myColors["0"],ann=F,...)
title(main= "harvesting capability wrt. g_skill value",xlab="value of g_skill",ylab= "maximum amount of harvested energy")
...
#	res.m=ddply(res.m,.(V4),transform,rescale=rescale(value))
	...
	#base_size <- 9
	 ggplot(res.m,aes(V4,V1))+geom_tile(aes(fill = value))+
		...
		scale_y_log10(expression(frac(harvest(Q[r1])+1),(harvest(Q[r0])+1)),col="black")+
		scale_x_continuous(expression(available(Q[r1])))+
		opts(
...
printASlice=function(heatmap,xlab,ylab,xlim,ylim,cols=c("white","blue"),...){
	...
	ramp=colorRampPalette(cols)(100)
 	image(x=as.numeric(rownames(heatmap)),y=as.numeric(colnames(heatmap)),z=heatmap,ylim=ylim,xlim=xlim,col=ramp,xlab=xlab,ylab=ylab,...)
}
