...
MedeaAgentObserver::MedeaAgentObserver( RobotAgentWorldModel *__wm )
{
	...
	_wm->_currentSigma = MedeaSharedData::gSigmaRef;
...
MedeaAgentObserver::~MedeaAgentObserver()
{
	// nothing to do.
}

void MedeaAgentObserver::reset()
{
	// nothing to do 
}

void MedeaAgentObserver::step()
{
	bool enableGenomeSpreading = true;
	...
	if(MedeaSharedData::gMaturityType == 1) {
		if( _wm->getActiveStatus() && _wm->getMaturity() > 0) {
			_wm->setMaturity(_wm->getMaturity()+1);
			if(_wm->getMaturity() > MedeaSharedData::gEnergyRevive +10 )
				_wm->setMaturity(0); //the robot will be able to spread is genomes again
		}
		enableGenomeSpreading &= _wm->getMaturity() <= 0;

	} else if(MedeaSharedData::gMaturityType == 2) {
		enableGenomeSpreading &= _wm->getMaturity() <= 0;
...
//	if ( _wm->getActiveStatus() == true )  	// broadcast only if agent is active (ie. not just revived) and deltaE>0.
	if ( _wm->getActiveStatus() == true && enableGenomeSpreading)  	// broadcast only if agent is active (ie. not just revived) and deltaE>0.
	{
		for (int i = 0 ; i < gAgentCounter ; i++)
		{
			if ( ( i != _wm->_agentId ) && ( gRadioNetworkArray[_wm->_agentId][i] ) ) //&& (_wm->getEnergyLevel() > 0.0) ) --> always true as status is active
			{
				MedeaAgentObserver* targetAgentObserver = dynamic_cast<MedeaAgentObserver*>(gWorld->getAgent(i)->getObserver());
				...
				if ( MedeaSharedData::gDynamicSigma == true )
				{
					float dice = float(rand() %100) / 100.0;
					float sigmaSendValue = 0.0;
					if ( ( dice >= 0.0 ) && ( dice < MedeaSharedData::gProbAdd) )
					{
						sigmaSendValue = _wm->_currentSigma * ( 1 + MedeaSharedData::gDynaStep );
						if (sigmaSendValue > MedeaSharedData::gSigmaMax)
						{
							sigmaSendValue = MedeaSharedData::gSigmaMax;
						}
					}
					else if ( ( dice >= MedeaSharedData::gProbAdd ) && ( dice < MedeaSharedData::gProbAdd+MedeaSharedData::gProbSub) )
					{
						sigmaSendValue = _wm->_currentSigma * ( 1 - MedeaSharedData::gDynaStep );
						if (sigmaSendValue < MedeaSharedData::gSigmaMin)
						{
							sigmaSendValue = MedeaSharedData::gSigmaMin;
						}
					}
					else
					{
						std::cerr << "Error : In SwarmOnlineObserver, the rand value is out of bound. The sum of MedeaSharedData::gProbRef and MedeaSharedData::gProbMax is probably not equal to one" << std::endl;
						exit(1);
					}
	...
	if( dynamic_cast<MedeaWorldObserver*>(gWorld->getWorldObserver())->getLifeIterationCount() >= MedeaSharedData::gEvaluationTime-1 ) // at the end of a generation
	{

		// Handle the maturity concept
		if(MedeaSharedData::gMaturityType == 2) {
			// Remove maturity from agent energy
			int diff = _wm->getEnergyLevel() - _wm->getMaturity();
			if(diff <= 0)
				diff = 0;
			_wm->setEnergyLevel(diff);
			_wm->setMaturity(0);
...
void MedeaAgentObserver::pickRandomGenome()
{
		if(_wm->_genomesList.size() != 0)
		{
...
void MedeaAgentObserver::writeGenome(std::vector<double> genome, int senderId, float sigma)
{
...
void MedeaAgentObserver::mutateWithBouncingBounds( float sigma)
{
	_wm->_genome.clear();
	...
	else
	{
		...
		if ( ( randValue >= 0.0 ) && ( randValue < MedeaSharedData::gProbRef) )
		{
			_wm->_currentSigma = MedeaSharedData::gSigmaRef;
		}
		else if ( ( randValue >= MedeaSharedData::gProbRef ) && ( randValue < MedeaSharedData::gProbMax+MedeaSharedData::gProbRef) )
		{
			_wm->_currentSigma = MedeaSharedData::gSigmaMax;
		}
		else
		{
			std::cerr << "Error : In SwarmOnlineObserver, the rand value is out of bound. The sum of MedeaSharedData::gProbRef and MedeaSharedData::gProbMax is probably not equal to one" << std::endl;
