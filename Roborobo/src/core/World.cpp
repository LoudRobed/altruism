...
void World::initWorld()
{
    ...
    */
	...
	for ( int i = 0 ; i != gNbOfAgents ; i++ )
	{
		if (gUseOrganisms)
		{
			RobotAgent* agent = new RobotAgentConnectable(this);
 ...
 * 		The keyboardStates, defaults to NULL if not set.
 */
void World::updateWorld(Uint8 *__keyboardStates)
{
	...
	if (gUseOrganisms && _setUpConnections) {
	...
	for (int i = 0; i != gAgentCounter; i++) {
		...
		if (gUseOrganisms && _setUpConnections) 
		{
			RobotAgentConnectable* currentAgent = dynamic_cast<RobotAgentConnectable*>(agents[i]);
			if (currentAgent == 0)
			{
				std::cerr << "Agent " << i << " isn't of type RobotAgentConnectabe" << std::endl;
				std::cerr << "When option gUseOrganisms is used, every agent should be of type RobotAgentConnectable " << std::endl;
				exit(1);
			}
			currentAgent->setUpConnections();
		}
	...
	for ( int i = 0 ; i < gAgentCounter ; i++ )
	{
		if ( gUseOrganisms )
		{
			RobotAgentConnectable* currentAgent = dynamic_cast<RobotAgentConnectable*>(agents[shuffledIndex[i]]);
			if (currentAgent == 0)
			{
				std::cerr << "Agent " << shuffledIndex[i] << " isn't of type RobotAgentConnectabe" << std::endl;
				std::cerr << "When option gUseOrganisms is used, every agent should be of type RobotAgentConnectable " << std::endl;
				exit(1);
			}

			if (currentAgent->isPartOfOrganism()) 
			{
				// Agents in an organism all move together
				Organism* organism = currentAgent->getOrganism();
				organism->move();
...
bool World::getInitiliazeAgents(){
    return _initializeAgents;
}

void World::setSetUpConnections(bool setUpConnections){
    _setUpConnections = setUpConnections;
}
