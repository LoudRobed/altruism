...
RobotAgent::RobotAgent( World *__world )
{
	...
	_worldInterface = gConfigurationLoader->make_RobotAgentWorldInterface(); 
...
void RobotAgent::applyDynamics() 
{
	...
	_worldInterface->applyInternalDynamics(_wm); //applyInternalDynamics is a static function 
...
{    
	...
	if ( gNiceRendering )
		apply_surface( _x - gCamera.x, _y - gCamera.y, this->_agentMask, gScreen ); // OPTIONAL (agent is already visible/registered through the environment image -- but: may be useful for image capture
...
//						putPixel32secure( gScreen, xTmp - gCamera.x, yTmp - gCamera.y , SDL_MapRGB( gScreen->format, 0xFF, 0x00, ranf() ) );
	...
	if ( gDisplaySensors == true )
	{
		...
		for ( int i = 0 ; i != _wm->_sensorCount ; i++ )
		{
			...
			double x1 = (_wm->_xReal + _wm->_sensors[i][1] * cos( _wm->_sensors[i][2] + _wm->_agentAbsoluteOrientation * M_PI / 180.0 ) ) ;
			double y1 = (_wm->_yReal + _wm->_sensors[i][1] * sin( _wm->_sensors[i][2] + _wm->_agentAbsoluteOrientation * M_PI / 180.0 ) ) ;
			double x2 = (_wm->_xReal + _wm->_sensors[i][3] * cos( _wm->_sensors[i][4] + _wm->_agentAbsoluteOrientation * M_PI / 180.0 ) ) ; 
			double y2 = (_wm->_yReal + _wm->_sensors[i][3] * sin( _wm->_sensors[i][4] + _wm->_agentAbsoluteOrientation * M_PI / 180.0 ) ) ; 
...
void RobotAgent::setAgentMask(SDL_Surface* mask)
{
    unregisterAgent();
		...
		_agentMaskData.clear();
    for ( int i = 0 ; i != mask->w ; i++ ) 
        for ( int j = 0 ; j != mask->h ; j++ )
            if ( getPixel32( mask , i , j) != SDL_MapRGB( gForegroundImage->format, 0xFF, 0xFF, 0xFF ) )
            {
								std::vector<int> coord;
								coord.resize(2);
								coord[0] = i;
								coord[1] = j;
								_agentMaskData.push_back(coord);
            }
		this->_agentMask = mask;
    registerAgent();
}

std::vector< std::vector<int> > RobotAgent::getAgentMaskData()
{
    return _agentMaskData;
}
...
void RobotAgent::registerAgent() {
	if (gAgentRegistration && !this->_registered) {
		...
		register_Mask(_x, _y, getAgentMaskData(), gEnvironmentImage, _wm->_agentId + 1000);
...
void RobotAgent::unregisterAgent() {
	if (gAgentRegistration && this->_registered) {
		clean_Mask(_x, _y, getAgentMaskData(), gEnvironmentImage);
