/*
 *  MedeaAltUtilityPerceptronControlArchitecture.cpp
 *  roborobo-online
 *
...
MedeaAltUtilityPerceptronControlArchitecture::MedeaAltUtilityPerceptronControlArchitecture( RobotAgentWorldModel *__wm )
{
	_wm = (MedeaAltUtilityAgentWorldModel*)__wm;
	//_wm->_genome.resize(18);
	_iteration = 0;

	//_initialEnergy = 2*MedeaAltUtilitySharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaAltUtilitySharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaAltUtilitySharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaAltUtilitySharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaAltUtilitySharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaAltUtilitySharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaAltUtilitySharedData::gEnergyInit);
	//_deltaEnergy = 10.0; // CEC
	_wm->setDeltaEnergy(0.0); // MedeaAltUtility
...
MedeaAltUtilityPerceptronControlArchitecture::~MedeaAltUtilityPerceptronControlArchitecture()
{
	// nothing to do.
}

void MedeaAltUtilityPerceptronControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaAltUtilityPerceptronControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaAltUtilitySharedData::gMaxKeyRange);
		return;
	}
*/
	...
	if ( _wm->getActiveStatus() == true )
	{
		...
		for (int j= 0 ; j < _nbHiddenNeurons ; j++ )
		{
			hiddenLayer[j] +=  (_wm->getEnergyLevel()/MedeaAltUtilitySharedData::gEnergyMax)  * _parameters[geneToUse];  // !N : added: energy value normalization btw 0 and 1.
			geneToUse ++;
		}
		...
		if ( MedeaAltUtilitySharedData::gExperimentNumber == 2 && ( (int)_wm->_xReal >= MedeaAltUtilitySharedData::g_xStart_EnergyZone && (int)_wm->_yReal >= MedeaAltUtilitySharedData::g_yStart_EnergyZone && (int)_wm->_xReal <= MedeaAltUtilitySharedData::g_xEnd_EnergyZone && (int)_wm->_yReal <= MedeaAltUtilitySharedData::g_yEnd_EnergyZone ) )
		{
			double penalizationFactor = ( 1.0 - MedeaAltUtilitySharedData::gMaxPenalizationRate )  + MedeaAltUtilitySharedData::gMaxPenalizationRate*(MedeaAltUtilitySharedData::gZoneEnergy_harvestValue/MedeaAltUtilitySharedData::gZoneEnergy_maxHarvestValue);  // based on MedeaAltUtilitySharedData::gZoneEnergy_harvestValue/MedeaAltUtilitySharedData::gZoneEnergy_maxHarvestValue
