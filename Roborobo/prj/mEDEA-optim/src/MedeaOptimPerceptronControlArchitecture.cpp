/*
 *  MedeaOptimPerceptronControlArchitecture.cpp
 *
...
MedeaOptimPerceptronControlArchitecture::MedeaOptimPerceptronControlArchitecture( RobotAgentWorldModel *__wm )
{
	_wm = (MedeaOptimAgentWorldModel*)__wm;
	//_wm->_genome.resize(18);
	_iteration = 0;

	//_initialEnergy = 2*MedeaOptimSharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaOptimSharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaOptimSharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaOptimSharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaOptimSharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaOptimSharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaOptimSharedData::gEnergyInit);
...
MedeaOptimPerceptronControlArchitecture::~MedeaOptimPerceptronControlArchitecture()
{
	// nothing to do.
}

void MedeaOptimPerceptronControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaOptimPerceptronControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaOptimSharedData::gMaxKeyRange);
		return;
	}
*/
	...
	if ( _wm->getActiveStatus() == true )
	{
		...
		for ( int i = 0 ; i < _wm->_sensorCount ; i++ )
		{
			for (int j= 0 ; j < _nbHiddenNeurons ; j++ )
			{
				hiddenLayer[j] += (_wm->getSensorDistanceValue(i)/_wm->getSensorMaximumDistanceValue(i)) * _parameters[geneToUse] ;
				geneToUse ++;
			}
		}
		...
		for (int j= 0 ; j < _nbHiddenNeurons ; j++ )
		{
			hiddenLayer[j] += _wm->getEnergyPointDirectionAngleValue()  * _parameters[geneToUse];
			geneToUse ++;
		}
		...
		for (int j= 0 ; j < _nbHiddenNeurons ; j++ )
		{
			hiddenLayer[j] += _wm->getEnergyPointDistanceValue()  * _parameters[geneToUse]; 
			geneToUse ++;
		}
		...
		for (int j= 0 ; j < _nbHiddenNeurons ; j++ )
		{
			hiddenLayer[j] +=  (_wm->getEnergyLevel()/MedeaOptimSharedData::gEnergyMax)  * _parameters[geneToUse];  // !N : added: energy value normalization btw 0 and 1.
			geneToUse ++;
		}
...
//		std::cout << "{" << _wm->_genome.size()-1 << "," << geneToUse << "}";
	}
}
