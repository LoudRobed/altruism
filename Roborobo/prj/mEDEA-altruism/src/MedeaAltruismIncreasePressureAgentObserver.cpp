/*
 *  MedeaAltruismIncreasePressureAgentObserver.cpp
 *  Roborobo
 *
...
MedeaAltruismIncreasePressureAgentObserver::MedeaAltruismIncreasePressureAgentObserver( RobotAgentWorldModel *__wm )
{
...
MedeaAltruismIncreasePressureAgentObserver::~MedeaAltruismIncreasePressureAgentObserver()
{
	// nothing to do.
}

void MedeaAltruismIncreasePressureAgentObserver::reset()
{
	_wm->_idlenessTracker.reset();
}

void MedeaAltruismIncreasePressureAgentObserver::step()
{
	// debug verbose
	...
	if ( _wm->getActiveStatus() == true && _wm->getMaturity()<=0)
	{
		for (int i = 0 ; i < gAgentCounter ; i++)
		{
			...
			{
				MedeaAltruismIncreasePressureAgentObserver* targetAgentObserver = dynamic_cast<MedeaAltruismIncreasePressureAgentObserver*>(gWorld->getAgent(i)->getObserver());
...
void MedeaAltruismIncreasePressureAgentObserver::checkGenomeList()
{
...
void MedeaAltruismIncreasePressureAgentObserver::loadGenome(std::vector<double> inGenome)
{
	std::cout << std::flush ;
...
void MedeaAltruismIncreasePressureAgentObserver::pickRandomGenome()
{
	if(_wm->_genomesList.size() != 0)
	{
...
void MedeaAltruismIncreasePressureAgentObserver::randomElitismGenomeSelection()
{
	if(_wm->_genomesList.size() != 0)
	{
...
void MedeaAltruismIncreasePressureAgentObserver::tournamentGenomeSelection()
{
	if(_wm->_genomesList.size() != 0)
	{
...
void MedeaAltruismIncreasePressureAgentObserver::writeGenome(std::vector<double> genome, int senderId, float sigma)
{
...
void MedeaAltruismIncreasePressureAgentObserver::mutateWithBouncingBounds( float sigma)
{
	if ( MedeaAltruismSharedData::gDynamicSigma == true)
	{
