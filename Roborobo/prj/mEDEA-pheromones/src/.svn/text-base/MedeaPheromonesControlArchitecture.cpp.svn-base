...
MedeaPheromonesControlArchitecture::MedeaPheromonesControlArchitecture( RobotAgentWorldModel *wm )
{
	_wm = (MedeaPheromonesAgentWorldModel*)wm;
	//_wm->_genome.resize(18);
	_iteration = 0;

	//_initialEnergy = 2*MedeaPheromonesSharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaPheromonesSharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaPheromonesSharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaPheromonesSharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaPheromonesSharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaPheromonesSharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaPheromonesSharedData::gEnergyInit);
...
void MedeaPheromonesControlArchitecture::createNN()
{
#if NNTYPE == NNTYPE_MLP
...
unsigned int MedeaPheromonesControlArchitecture::computeRequiredNumberOfWeights(MedeaPheromonesAgentWorldModel *wm)
{
	NeuralNetwork *a;
...
MedeaPheromonesControlArchitecture::~MedeaPheromonesControlArchitecture()
{
	delete nn;
	nn = NULL;
}

void MedeaPheromonesControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaPheromonesControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaPheromonesSharedData::gMaxKeyRange);
		return;
	}
*/
	...
	if ( _wm->getActiveStatus() == true )
	{
		...
		if ( MedeaPheromonesSharedData::gExperimentNumber == 2)
		{
...
//			Point2d closestPoint = gEnergyPoints[MedeaPheromonesSharedData::gActiveSun].getPosition();
//
...
//		if ( gAgentIndexFocus == _wm->_agentId )
		...
		// energy level
		(*inputs)[inputToUse++] = _wm->getEnergyLevel() / MedeaPheromonesSharedData::gEnergyMax;
		...
		if ( MedeaPheromonesSharedData::gExperimentNumber == 2 && ( (int)_wm->_xReal >= MedeaPheromonesSharedData::g_xStart_EnergyZone && (int)_wm->_yReal >= MedeaPheromonesSharedData::g_yStart_EnergyZone && (int)_wm->_xReal <= MedeaPheromonesSharedData::g_xEnd_EnergyZone && (int)_wm->_yReal <= MedeaPheromonesSharedData::g_yEnd_EnergyZone ) )
		{
			double penalizationFactor = ( 1.0 - MedeaPheromonesSharedData::gMaxPenalizationRate )  + MedeaPheromonesSharedData::gMaxPenalizationRate*(MedeaPheromonesSharedData::gZoneEnergy_harvestValue/MedeaPheromonesSharedData::gZoneEnergy_maxHarvestValue);  // based on MedeaPheromonesSharedData::gZoneEnergy_harvestValue/MedeaPheromonesSharedData::gZoneEnergy_maxHarvestValue
