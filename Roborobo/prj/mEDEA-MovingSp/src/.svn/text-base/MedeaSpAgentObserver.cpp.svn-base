...
// 		std::cout << "target: " << _wm->getEnergyPointDirectionAngleValue() << std::endl;
	...
	if ( _wm->getActiveStatus() && _wm->getMaturity()<=0) // TOCHECK
	{
		for (int i = 0 ; i < gAgentCounter ; i++)
		{
			if ( ( i != _wm->_agentId ) && ( gRadioNetworkArray[_wm->_agentId][i] ) ) //&& (_wm->getEnergyLevel() > 0.0) ) --> always true as status is active
			{
				...
				//	_wm->setNbMetRobots(_wm->getNbMetRobots()+1);
...
void MedeaSpAgentObserver::writeGenome(std::vector<double> genome, int senderId, float sigma)
{
