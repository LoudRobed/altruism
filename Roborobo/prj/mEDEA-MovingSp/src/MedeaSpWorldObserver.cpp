...
#include "mEDEA-MovingSp/include/MedeaSpNetworkGenerator.h"
	...
	if( _lifeIterationCount >= (MedeaSpSharedData::gEvaluationTime) ) // print mEDEA state.
	  {
	    ...
	    int both=0;
	    int impGen=0;
	    int harv0 = 0; int harv1 = 0;
	    //double density = 0.0;
	    for ( int i = 0 ; i != gAgentCounter ; i++ )
	      {
		MedeaSpAgentWorldModel * _wm = (dynamic_cast<MedeaSpAgentWorldModel*>(gWorld->getAgent(i)->getWorldModel()));
		if ( _wm->getActiveStatus() && (_wm->getEnergyLevel() > 0))
		  {
		    //std::cout << "Lifetime = " << _wm->getLifeTime() << "\n";
		    impGen += _wm->_genomesList.size();
		    activeCount++;
		    //	    density += ((double)_wm->getNbMetRobots()) / ((double)MedeaSpSharedData::gEvaluationTime);
		    //_wm->setNbMetRobots(0);
			...
			for(uint i=0; i < (*it)->harvestersList.size(); i++)
			  {
			    if(_wm->_agentId ==  (*it)->harvestersList.at(i))
			      {
		    ...
		    if(insideSomething == false)
		      {
			std::cout << "Agent " << _wm->_agentId << "  energyLevel = " << _wm->getEnergyLevel() << " maturity = " << _wm->getMaturity() << " lifetime = "<< _wm->getLifeTime() << "\n";
		      }
		    */
		  }
	      }
	    ...
	    for(std::vector<MedeaSpEnergyPoint*>::iterator it = coopEnergyPoints->begin(); it<coopEnergyPoints->end(); it++) 
		{
			...
			if( (*it)->getType() == 0 )
			{
			    harv0 = (*it)->harvestersList.size();
			}
			if( (*it)->getType() == 1 )
			{
			    harv1 = (*it)->harvestersList.size();
			}
		}
	    ...
	    gLogFile << gWorld->getIterations() << " : harvesters " << harv0 << " " << harv1 << std::endl;
	    //   gLogFile << gWorld->getIterations() << " : average met robots by step " << density << std::endl;
	    // std::cout << "impGen = " << impGen << "\n";
	    double averageGL = ((double) impGen) / ((double) gAgentCounter);
	    gLogFile << gWorld->getIterations() << " : averageImportation " << averageGL << std::endl;
		...
		// update harvestersLists
		
		for(std::vector<MedeaSpEnergyPoint*>::iterator it = coopEnergyPoints->begin(); it != coopEnergyPoints->end(); it++)
		{
		    //std::cout << "\nType" << (*it)->getType() << " -> HList = " << (*it)->harvestersList.size() << "\n";
		    if(gVerbose) // debug
			{
			  std::cout << "[";
			  for(uint i=0; i<(*it)->harvestersList.size(); i++)
			  {
			      std::cout << (*it)->harvestersList.at(i) << "   ";
			  }
			  std::cout << "]\n";
			}
			
			  (*it)->harvestersList.clear();
			  //std::cout << "\nType" << (*it)->getType() << " -> HList = " << (*it)->harvestersList.size() << "\n";
		}
...
double  MedeaSpWorldObserver::forageReward(double G, double Q_E ){
	...
	if(th)
	{
		...
		//std::cout << "G = " << G << "   Q_E = " << Q_E << " -> reward = " << reward << "\n";
	}
	else
	{
		n=n;
...
void MedeaSpWorldObserver::updateEnvironmentResources()
{
	...
	if ( gWorld->getIterations() % ( int(MedeaSpSharedData::gSunLifetime) ) == 0 && !MedeaSpSharedData::gExperimentNoMovements )
	{
		...
		/********* Partie changée par rapport à Simon *********/
		else 
		{
			...
			bool fixSun = true; 	// For fixed suns
			if(fixSun){
				x1=512;
				y1=245;
				x0=512 ;
	...
	for(std::vector<MedeaSpEnergyPoint*>::iterator it = coopEnergyPoints->begin(); it<coopEnergyPoints->end(); it++) 
	{
		// std::cout << "NHarverst = "<< (*it)->getNHarvest() << "\n";
		// std::cout << "Q_E = "<< (*it)->getQ_E() << "\n";
		if((*it)->getType()==1)
...
{	
	...
	for ( int i = 0 ; i != gAgentCounter ; i++ ) // for each agent
	{
		...
		if(true) /*(gWorld->getIterations() > MedeaSpSharedData::gNoEnergy)*/{
			if ( currentAgentWorldModel->getActiveStatus() == true )
			{
				...
				for(std::vector<MedeaSpEnergyPoint*>::iterator it = coopEnergyPoints->begin(); it<coopEnergyPoints->end(); it++) 
				{
					//std::cout << (*it)->harvestersList.size() << "\n";
					if(true) // (getEuclidianDistance (posRobot,(*it)->getPosition()) < gEnergyPointRadius) && (*it)->getActiveStatus())
					{
						...
						*/
						...
						/** Checking if new robots are allowed in harvesting current ressource **/
						
						
						int id = currentAgentWorldModel->_agentId;
						bool inside = false;
						if((*it)->harvestersList.size() >= abs(MedeaSpSharedData::gNbAllowedRobotsBySun))  // ressource is full of robots
						{
						    for(uint i=0;i<(*it)->harvestersList.size();i++) 
						      {
							  if(((*it)->harvestersList.at(i)) == id) {inside = true;} // checking if the robot is already in the list of harvesters
						      }
						      if(inside == false) {energyReachable = 0;}    // if the robot is not already inside, it can't harvest
						}
						else        // ressource is not full of robots
						{
						    if(energyReachable > 0)    // the robot is able to harvest this kind of ressource
						    {
						      if((*it)->harvestersList.size() > 0) 
						      {
							for(uint i=0;i<(*it)->harvestersList.size();i++) 
							{
							    if(((*it)->harvestersList.at(i)) == id) {inside = true;} // checking if the robot is already in the list of harvesters
						      ...
						      if(!inside) { (*it)->harvestersList.push_back(id); }
						    }
						}
						...
						if(currentAgentWorldModel->getEnergyLevel() + energyReachable > MedeaSpSharedData::gEnergyMax)
						{
						    energyAdded = MedeaSpSharedData::gEnergyMax - currentAgentWorldModel->getEnergyLevel();
						    currentAgentWorldModel->setEnergyLevel(MedeaSpSharedData::gEnergyMax);
						}
						else
						{
						    energyAdded = energyReachable;
						    currentAgentWorldModel->setEnergyLevel(currentAgentWorldModel->getEnergyLevel() + energyAdded);
						}
						
						//std::cout << (*it)->getType() << "  Counter = " << currentAgentWorldModel->getEnergyCounterOfOneRessource((*it)->getType()) << "\n";
						/* Try */
						//if (energyReachable < 0.01) { energyAdded = 0;}
						...
						//std::cout << "REACHABLE = " << energyReachable << "   ADDED = " << energyAdded << "\n";
...
// 						currentAgentWorldModel->setRobotLED_status(false);
			...
			//std::cout << "ENERGY LEVEL = " << currentAgentWorldModel->getEnergyLevel() << "\n";
			
			if  ( (currentAgentWorldModel->getEnergyLevel()  <= 0)  && currentAgentWorldModel->getActiveStatus() )
			{
