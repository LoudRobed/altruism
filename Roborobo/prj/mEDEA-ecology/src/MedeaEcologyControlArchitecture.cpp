...
MedeaEcologyControlArchitecture::MedeaEcologyControlArchitecture( RobotAgentWorldModel *wm )
{
	_wm = (MedeaEcologyAgentWorldModel*)wm;
	//_wm->_genome.resize(18);
	_iteration = 0;

	//_initialEnergy = 2*MedeaEcologySharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaEcologySharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaEcologySharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaEcologySharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaEcologySharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaEcologySharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaEcologySharedData::gEnergyInit);
...
void MedeaEcologyControlArchitecture::createNN()
{
#if NNTYPE == NNTYPE_MLP
...
unsigned int MedeaEcologyControlArchitecture::computeRequiredNumberOfWeights(MedeaEcologyAgentWorldModel *wm)
{
	NeuralNetwork *a;
...
MedeaEcologyControlArchitecture::~MedeaEcologyControlArchitecture()
{
	delete nn;
	nn = NULL;
}

void MedeaEcologyControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaEcologyControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaEcologySharedData::gMaxKeyRange);
		return;
	}
*/
	...
	if ( _wm->getActiveStatus() == true )
	{
		...
		if ( MedeaEcologySharedData::gExperimentNumber == 2)
		{
...
//			Point2d closestPoint = gEnergyPoints[MedeaEcologySharedData::gActiveSun].getPosition();
//
...
//		if ( gAgentIndexFocus == _wm->_agentId )
		...
		// energy level
		(*inputs)[inputToUse++] = _wm->getEnergyLevel() / MedeaEcologySharedData::gEnergyMax;
		...
		if ( MedeaEcologySharedData::gExperimentNumber == 2 && ( (int)_wm->_xReal >= MedeaEcologySharedData::g_xStart_EnergyZone && (int)_wm->_yReal >= MedeaEcologySharedData::g_yStart_EnergyZone && (int)_wm->_xReal <= MedeaEcologySharedData::g_xEnd_EnergyZone && (int)_wm->_yReal <= MedeaEcologySharedData::g_yEnd_EnergyZone ) )
		{
			double penalizationFactor = ( 1.0 - MedeaEcologySharedData::gMaxPenalizationRate )  + MedeaEcologySharedData::gMaxPenalizationRate*(MedeaEcologySharedData::gZoneEnergy_harvestValue/MedeaEcologySharedData::gZoneEnergy_maxHarvestValue);  // based on MedeaEcologySharedData::gZoneEnergy_harvestValue/MedeaEcologySharedData::gZoneEnergy_maxHarvestValue
