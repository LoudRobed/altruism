...
//#define NNTYPE				NNTYPE_PERCEPTRON
#define NNTYPE				NNTYPE_ELMAN
//#define NNTYPE				NNTYPE_MLP
//#define NNTYPE				NNTYPE_PUNN
...
MedeaHGTNNControlArchitecture::MedeaHGTNNControlArchitecture( RobotAgentWorldModel *wm )
{
	_wm = (MedeaHGTAgentWorldModel*)wm;
	//_wm->_genome.resize(18);
	_iteration = 0;

	//_initialEnergy = 2*MedeaHGTSharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaHGTSharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaHGTSharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaHGTSharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaHGTSharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaHGTSharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaHGTSharedData::gEnergyInit);
	...
	// TODO
	// Initialize population ID
	_wm->setPopulationID(0);
...
void MedeaHGTNNControlArchitecture::createNN()
{
#if NNTYPE == NNTYPE_MLP
...
unsigned int MedeaHGTNNControlArchitecture::computeRequiredNumberOfWeights(MedeaHGTAgentWorldModel *wm)
{
	NeuralNetwork *a;
...
MedeaHGTNNControlArchitecture::~MedeaHGTNNControlArchitecture()
{
	delete nn;
	nn = NULL;
}

void MedeaHGTNNControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaHGTNNControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaHGTSharedData::gMaxKeyRange);
		return;
	}
*/
	...
	if ( _wm->getActiveStatus() == true )
	{
		...
		if ( MedeaHGTSharedData::gExperimentNumber == 2)
		{
...
//			Point2d closestPoint = gEnergyPoints[MedeaHGTSharedData::gActiveSun].getPosition();
//
...
//		if ( gAgentIndexFocus == _wm->_agentId )
		...
		// energy level
		(*inputs)[inputToUse++] = _wm->getEnergyLevel() / MedeaHGTSharedData::gEnergyMax;
		...
		if ( MedeaHGTSharedData::gExperimentNumber == 2 && ( (int)_wm->_xReal >= MedeaHGTSharedData::g_xStart_EnergyZone && (int)_wm->_yReal >= MedeaHGTSharedData::g_yStart_EnergyZone && (int)_wm->_xReal <= MedeaHGTSharedData::g_xEnd_EnergyZone && (int)_wm->_yReal <= MedeaHGTSharedData::g_yEnd_EnergyZone ) )
		{
			double penalizationFactor = ( 1.0 - MedeaHGTSharedData::gMaxPenalizationRate )  + MedeaHGTSharedData::gMaxPenalizationRate*(MedeaHGTSharedData::gZoneEnergy_harvestValue/MedeaHGTSharedData::gZoneEnergy_maxHarvestValue);  // based on MedeaHGTSharedData::gZoneEnergy_harvestValue/MedeaHGTSharedData::gZoneEnergy_maxHarvestValue
