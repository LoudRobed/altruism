/*
 *  MedeaTwoSunsPerceptronControlArchitecture.cpp
 *  roborobo-online
 *
...
MedeaTwoSunsPerceptronControlArchitecture::MedeaTwoSunsPerceptronControlArchitecture( RobotAgentWorldModel *__wm )
{
	_wm = (MedeaTwoSunsAgentWorldModel*)__wm;
	//_wm->_genome.resize(18);
	_iteration = 0;

	//_initialEnergy = 2*MedeaTwoSunsSharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaTwoSunsSharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaTwoSunsSharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaTwoSunsSharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaTwoSunsSharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaTwoSunsSharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaTwoSunsSharedData::gEnergyInit);
	//_deltaEnergy = 10.0; // CEC
	_wm->setDeltaEnergy(0.0); // MedeaTwoSuns
...
MedeaTwoSunsPerceptronControlArchitecture::~MedeaTwoSunsPerceptronControlArchitecture()
{
	// nothing to do.
}

void MedeaTwoSunsPerceptronControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaTwoSunsPerceptronControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaTwoSunsSharedData::gMaxKeyRange);
		return;
	}
*/
	...
	if ( _wm->getActiveStatus() == true )
	{
		...
		else
		{
			...
			Point2d closestPoint = gEnergyPoints[MedeaTwoSunsSharedData::gActiveSun].getPosition();
...
//		if ( gAgentIndexFocus == _wm->_agentId )
		...
		for (int j= 0 ; j < _nbHiddenNeurons ; j++ )
		{
			hiddenLayer[j] +=  (_wm->getEnergyLevel()/MedeaTwoSunsSharedData::gEnergyMax)  * _parameters[geneToUse];  // !N : added: energy value normalization btw 0 and 1.
			geneToUse ++;
		}
		...
		if ( MedeaTwoSunsSharedData::gExperimentNumber == 2 && ( (int)_wm->_xReal >= MedeaTwoSunsSharedData::g_xStart_EnergyZone && (int)_wm->_yReal >= MedeaTwoSunsSharedData::g_yStart_EnergyZone && (int)_wm->_xReal <= MedeaTwoSunsSharedData::g_xEnd_EnergyZone && (int)_wm->_yReal <= MedeaTwoSunsSharedData::g_yEnd_EnergyZone ) )
		{
			double penalizationFactor = ( 1.0 - MedeaTwoSunsSharedData::gMaxPenalizationRate )  + MedeaTwoSunsSharedData::gMaxPenalizationRate*(MedeaTwoSunsSharedData::gZoneEnergy_harvestValue/MedeaTwoSunsSharedData::gZoneEnergy_maxHarvestValue);  // based on MedeaTwoSunsSharedData::gZoneEnergy_harvestValue/MedeaTwoSunsSharedData::gZoneEnergy_maxHarvestValue
