...
MedeaTwoSunsAgentObserver::MedeaTwoSunsAgentObserver( RobotAgentWorldModel *__wm )
{
	...
	_wm->_currentSigma = MedeaTwoSunsSharedData::gSigmaRef;
...
MedeaTwoSunsAgentObserver::~MedeaTwoSunsAgentObserver()
{
	// nothing to do.
}

void MedeaTwoSunsAgentObserver::reset()
{
	// nothing to do 
}

void MedeaTwoSunsAgentObserver::step()
{
	// debug verbose
	...
	if ( _wm->getActiveStatus() == true )  	// broadcast only if agent is active (ie. not just revived) and deltaE>0.
	{
		for (int i = 0 ; i < gAgentCounter ; i++)
		{
			...
			{
				MedeaTwoSunsAgentObserver* targetAgentObserver = dynamic_cast<MedeaTwoSunsAgentObserver*>(gWorld->getAgent(i)->getObserver());
				...
				if ( MedeaTwoSunsSharedData::gDynamicSigma == true )
				{
					float dice = float(rand() %100) / 100.0;
					float sigmaSendValue = 0.0;
					if ( ( dice >= 0.0 ) && ( dice < MedeaTwoSunsSharedData::gProbAdd) )
					{
						sigmaSendValue = _wm->_currentSigma * ( 1 + MedeaTwoSunsSharedData::gDynaStep );
						if (sigmaSendValue > MedeaTwoSunsSharedData::gSigmaMax)
						{
							sigmaSendValue = MedeaTwoSunsSharedData::gSigmaMax;
						}
					}
					else if ( ( dice >= MedeaTwoSunsSharedData::gProbAdd ) && ( dice < MedeaTwoSunsSharedData::gProbAdd+MedeaTwoSunsSharedData::gProbSub) )
					{
						sigmaSendValue = _wm->_currentSigma * ( 1 - MedeaTwoSunsSharedData::gDynaStep );
						if (sigmaSendValue < MedeaTwoSunsSharedData::gSigmaMin)
						{
							sigmaSendValue = MedeaTwoSunsSharedData::gSigmaMin;
						}
					}
					else
					{
						std::cerr << "Error : In SwarmOnlineObserver, the rand value is out of bound. The sum of MedeaTwoSunsSharedData::gProbRef and MedeaTwoSunsSharedData::gProbMax is probably not equal to one" << std::endl;
						exit(1);
					}
	...
	if( dynamic_cast<MedeaTwoSunsWorldObserver*>(gWorld->getWorldObserver())->getLifeIterationCount() >= MedeaTwoSunsSharedData::gEvaluationTime-1 ) // at the end of a generation
	{
		...
		if ( MedeaTwoSunsSharedData::gExperimentNumber == 1 || MedeaTwoSunsSharedData::gExperimentNumber == 2 )
		{
...
void MedeaTwoSunsAgentObserver::pickRandomGenome()
{
		if(_wm->_genomesList.size() != 0)
		{
...
void MedeaTwoSunsAgentObserver::writeGenome(std::vector<double> genome, int senderId, float sigma)
{
...
void MedeaTwoSunsAgentObserver::mutateWithBouncingBounds( float sigma)
{
	_wm->_genome.clear();
	...
	else
	{
		...
		if ( ( randValue >= 0.0 ) && ( randValue < MedeaTwoSunsSharedData::gProbRef) )
		{
			_wm->_currentSigma = MedeaTwoSunsSharedData::gSigmaRef;
		}
		else if ( ( randValue >= MedeaTwoSunsSharedData::gProbRef ) && ( randValue < MedeaTwoSunsSharedData::gProbMax+MedeaTwoSunsSharedData::gProbRef) )
		{
			_wm->_currentSigma = MedeaTwoSunsSharedData::gSigmaMax;
		}
		else
		{
			std::cerr << "Error : In SwarmOnlineObserver, the rand value is out of bound. The sum of MedeaTwoSunsSharedData::gProbRef and MedeaTwoSunsSharedData::gProbMax is probably not equal to one" << std::endl;
